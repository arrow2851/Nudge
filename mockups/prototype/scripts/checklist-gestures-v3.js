...
function modelFor(context, state = store.getState()) {
  if (context.kind === 'task') {
    return {
      items: state.tasks.filter(item => item.type === 'task' && !item.archived),
...
function saveItems(context, current, items, label) {
  if (context.kind === 'task') {
    ...
    store.setState({
      tasks: current.tasks.map(item => updated.get(item.id) || item),
...
function beginDrag() {
  ...
  const nodeStyle = {
    ...
    height: `${rect.height}px`, margin: '0', zIndex: '999', pointerEvents: 'none'
  };
...
function animateLayout(before) {
  requestAnimationFrame(() => {
    before.forEach((oldRect, node) => {
      ...
      const delta = oldRect.top - node.getBoundingClientRect().top;
...
function updateDrag(event) {
  ...
  event.preventDefault();
  gesture.context.dragNode.style.transform = `translate3d(${event.clientX - gesture.startX}px, ${event.clientY - gesture.startY}px, 0)`;
...
document.addEventListener('pointerdown', event => {
  ...
  ui.gesture = {
    context, pointerId: event.pointerId, startX: event.clientX, startY: event.clientY,
    mode: '', distance: 0, targetId: '', placeAfter: false, armed: false,
