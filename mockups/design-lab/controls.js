...
const INTERACTIVE_LOOKS = new Set([2, 3, 4, 5, 6, 7]);
...
export function renderReviewControls(state, data, elements) {
  ...
  document.querySelector('#scenario-purpose').textContent = `${data.label}: ${data.purpose}${!INTERACTIVE_LOOKS.has(state.look) ? ' Today interaction is currently implemented in Looks #2 through #7.' : ''}`;
