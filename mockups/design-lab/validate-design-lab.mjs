...
function fixturesAndRoutes(shared) {
  ...
  check(shared.defaultState().look === 5 && shared.defaultState().view === 'tasks', 'Default review route is not Look #5 Tasks.');
...
function taskHierarchyContract() {
  ...
  check(app.includes('new Set([3, 4, 5])'), 'app.js does not register Looks #3, #4, and #5 for Task hierarchy.');
  check(app.includes('TASK_RENDERERS'), 'app.js is missing the task renderer registry.');
  check(controls.includes('new Set([3, 4, 5])'), 'controls.js does not register Looks #3, #4, and #5 for Task hierarchy guidance.');
  ...
  passes.push('Checked shared Task hierarchy state and Looks #3/#4/#5 renderer, action, drag, responsive, and accessibility contracts.');
}
...
function main() {
  ...
  else console.log('\nAll gallery, Routine Completion, and Looks #3/#4/#5 Task hierarchy checks passed.');
}
