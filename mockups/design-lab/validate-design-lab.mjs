...
const INTERVENTION_LOOKS = [...LOOKS];
...
const interventionRenderers = new Map([
  [2, ['renderers/look2-intervention.js', 'renderInterventionEditorialAction', 'look2-intervention.css', ['.ed-intervention-entry', '.ed-intervention-facts', 'max-height: 720px']]],
  ...
  [9, ['renderers/look9-intervention.js', 'renderInterventionRetroAction', 'look9-intervention.css', ['.rd-intervention-record', '.rd-intervention-facts', 'max-height: 720px', 'prefers-contrast: more']]]
]);

const styles = [
  'styles.css', 'foundation.css', 'look2-interactive.css', 'look2-tasks.css', 'look2-intervention.css',
  ...
  'look9-quality.css', 'look9-interactive.css', 'look9-tasks.css', 'look9-intervention.css', 'review.css'
];
...
function fixturesAndRoutes(shared) {
  ...
  check(shared.defaultState().look === 9 && shared.defaultState().view === 'intervention', 'Default review route is not Look #9 Intervention.');
...
function sharedRoutineContract() {
  ...
  ['applyRoutineState', 'completeRoutine', 'reopenRoutine', 'clearInteractiveState'].forEach(name => check(app.includes(name), `app.js does not use ${name}.`));
  ['routine-completion-v1', 'nextLabel', 'previousStatus', 'nextStatus'].forEach(token => check(interaction.includes(token), `interactive-state.js is missing ${token}.`));
...
function lookRoutineContract(look, rendererFile, cssFile, tokens) {
  ...
  ['data-action="complete-routine"', 'data-action="reopen-routine"', 'data-section-id', 'data-chore-id'].forEach(token => check(renderer.includes(token), `Look #${look} renderer is missing ${token}.`));
  ['forced-colors: active', 'prefers-reduced-motion: reduce', ...tokens].forEach(token => check(css.includes(token), `Look #${look} interactive CSS is missing ${token}.`));
...
function taskHierarchyContract() {
  ...
  ['nudge-design-lab-task-hierarchy-v1', 'addTask', 'updateTaskTitle', 'toggleTaskCompletion', 'toggleMainTask', 'moveTask', 'moveTaskBefore', 'indentTask', 'unindentTask', 'setHideCompleted', 'taskProgress', 'released', 'syncParent'].forEach(token => check(taskState.includes(token), `task-state.js is missing ${token}.`));
  for (const [look, [rendererFile, exportName, cssFile, cssTokens]] of taskRenderers) {
    ...
    ['data-action="add-task-top"', 'data-action="add-task-bottom"', 'data-action="add-subtask"', 'data-action="toggle-task-completion"', 'data-action="toggle-main-task"', 'data-action="indent-task"', 'data-action="unindent-task"', 'data-task-drag="true"', 'data-task-title="true"', 'taskProgress'].forEach(token => check(renderer.includes(token), `Look #${look} task renderer is missing ${token}.`));
    ['forced-colors: active', 'prefers-reduced-motion: reduce', 'min-height: 48px', ...cssTokens].forEach(token => check(css.includes(token), `Look #${look} task CSS is missing ${token}.`));
  }
...
function interventionContract() {
  ...
  check(app.includes('new Set([2, 3, 4, 5, 6, 7, 8, 9])'), 'app.js does not register every active Look for Intervention-to-action.');
  check(controls.includes('new Set([2, 3, 4, 5, 6, 7, 8, 9])'), 'controls.js does not register every active Look for Intervention guidance.');
  ...
  for (const [look, [rendererFile, exportName, cssFile, cssTokens]] of interventionRenderers) {
    ...
    check(new RegExp(`export\\s+function\\s+${exportName}\\b`).test(renderer), `${rendererFile} does not export ${exportName}.`);
    check(app.includes(exportName), `app.js does not reference ${exportName}.`);
    ['data-action="start-intervention"', 'data-action="next-intervention"', 'data-action="dismiss-intervention"', 'data-action="resume-intervention"', 'data-action="complete-intervention"', 'data-action="reopen-intervention"', 'data-action="undo-intervention"', 'data-action="return-today"'].forEach(token => check(renderer.includes(token), `Look #${look} Intervention renderer is missing ${token}.`));
    ['min-height: 48px', 'forced-colors: active', 'prefers-reduced-motion: reduce', ...cssTokens].forEach(token => check(css.includes(token), `Look #${look} Intervention CSS is missing ${token}.`));
  }

  const editorial = read('renderers/look2-intervention.js').toUpperCase();
  ['EITHER CHOICE IS COMPLETE', 'NO REFLECTION, JOURNAL ENTRY, SCORE, REMINDER, OR REQUIREMENT', 'NOTHING NEEDS TO BE WRITTEN OR EXPLAINED'].forEach(token => check(editorial.includes(token), `Warm Editorial no-journaling language is missing ${token}.`));
  ['<TEXTAREA', 'CONTENTEDITABLE=', 'NAME="REFLECTION"', 'NAME="JOURNAL"'].forEach(token => check(!editorial.includes(token), `Warm Editorial renderer contains an unapproved writing input: ${token}.`));

  const tactile = read('renderers/look6-intervention.js').toUpperCase();
  ['EITHER CHOICE IS COMPLETE', 'NO TIMER, SCORE, REMINDER, FOLLOW-UP, OR REQUIREMENT', 'NO ACTION WAS STARTED', 'NOTHING IS OWED'].forEach(token => check(tactile.includes(token), `Tactile Household optional-choice language is missing ${token}.`));
  ['BROKEN', 'DEFECT', 'FAILED', 'REPAIR REQUIRED', 'SERVICE GRADE', 'SERVICE NOTE'].forEach(token => check(!tactile.includes(token), `Tactile Household renderer contains prohibited defect language: ${token}.`));
  ...
  const retro = read('renderers/look9-intervention.js').toUpperCase();
  ['BOTH CHOICES ARE VALID', 'FIXTURE SNAPSHOT', 'NOT A LIVE TIMER', 'NO ACTION WAS STARTED', 'NOTHING IS OWED'].forEach(token => check(retro.includes(token), `Retro Digital neutral-system language is missing ${token}.`));
  ['ERROR', 'FAILURE', 'FAULT', 'ALARM', 'WARNING', 'FAILED PROCESS'].forEach(token => check(!retro.includes(token), `Retro Digital renderer contains prohibited failure token: ${token}.`));

  passes.push('Checked shared Intervention phases, deterministic alternatives, all reversible actions, all eight pure-Look renderers, language boundaries, transparency fallbacks, contrast, and accessibility contracts.');
}
...
function main() {
  ...
  else console.log('\nAll gallery, Routine Completion, Task hierarchy, and all eight Intervention-to-action checks passed.');
}
